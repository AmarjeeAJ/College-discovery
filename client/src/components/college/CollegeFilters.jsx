...
export default function CollegeFilters({ filters, onFilterChange, onReset }) {
  ...
  return (
    <div className="bg-white border border-brand-border rounded p-5 space-y-6">
      ...
      {/* Advisory Callout */}
      <div className="pt-2 border-t border-brand-border text-xs text-brand-gray bg-brand-teal-light/40 p-3 rounded">
        <span className="font-semibold text-brand-teal-dark block mb-1">
          Need Guidance?
        </span>
        Speak with a senior counselor for personalized seat shortlisting based on your rank.
      </div>
    </div>
  );
}
