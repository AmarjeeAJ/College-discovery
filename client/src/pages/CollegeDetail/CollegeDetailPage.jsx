...
export default function CollegeDetailPage() {
  ...
  useEffect(() => {
    async function loadCollege() {
      setLoading(true);
      const res = await apiService.getCollegeBySlug(slug);
      if (res.success) {
        setCollege(res.data);
        // Fetch similar colleges
        const allRes = await apiService.getColleges({ stream: res.data.stream[0] });
        if (allRes.success) {
          setSimilarColleges(allRes.data.filter((c) => c.id !== res.data.id).slice(0, 3));
        }
      }
      setLoading(false);
      window.scrollTo(0, 0);
  ...
  return (
    <div className="space-y-8 pb-16">
      <SEO
        title={`${college.name} (${college.shortName}): Fees, Placements, Cutoffs & Admissions 2026`}
        description={`Explore complete verified details for ${college.name} in ${college.city}. Check NIRF rank #${college.nirfRanking}, average package ${formatPackage(college.averagePackage)}, course fee schedules, and admission eligibility.`}
        canonical={`https://theshineeducation.com/college/${college.slug}`}
        schema={{
          ...
          "url": `https://theshineeducation.com/college/${college.slug}`
        }}
      />
      ...
      {/* =========================================================================
          ...
          ========================================================================= */}
      <div className="sticky top-18 z-20 bg-white border-b border-brand-border shadow-subtle">
          ...
          <main className="lg:col-span-8 space-y-10">
              ...
              <section className="space-y-6">
                ...
                <div className="space-y-4">
                  {college.courses.map((course) => (
                    <div key={course.id} className="border border-brand-border rounded p-4 bg-white space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-brand-border/60 pb-2.5">
                        ...
                        <div className="text-right sm:text-right">
                          ...
                          <span className="font-extrabold text-brand-dark text-base text-brand-teal-dark">
                            {course.annualFee}
                          </span>
                        </div>
                      </div>
                ...
                <div>
                  ...
                  <ol className="space-y-3 list-none p-0">
                    {college.admissionProcess.map((step, idx) => (
                      <li key={idx} className="flex items-start gap-3 text-xs sm:text-sm text-brand-gray">
                        <span className="w-6 h-6 rounded-sm bg-brand-teal text-white flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                          {idx + 1}
                        </span>
            ...
            {activeTab === 'cutoffs' && (
              <section className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-brand-dark mb-2">
                    Cutoff Trends & NIRF Parameters
                  </h2>
                  <p className="text-xs text-brand-gray">
                    Opening and closing ranks for major branches from the recent admission counseling session.
                  </p>
                </div>
