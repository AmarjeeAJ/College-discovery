...
export default function StreamCollegesPage() {
  const location = useLocation();
  const path = location.pathname.toLowerCase();

  // Determine which stream this page represents
  let streamKey = 'btech';
  if (path.includes('engineering')) streamKey = 'engineering';
  else if (path.includes('mba')) streamKey = 'mba';
  else if (path.includes('medical')) streamKey = 'medical';
  else if (path.includes('law')) streamKey = 'law';
  else if (path.includes('management')) streamKey = 'management';
  else if (path.includes('universities')) streamKey = 'universities';
  ...
  useEffect(() => {
    async function loadStreamColleges() {
      setLoading(true);
      try {
        const res = await apiService.getColleges({ stream: meta.streamFilter });
        ...
        console.error("Failed to load stream colleges:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Dynamic filter criteria tailored to discipline
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      <SEO
        ...
        canonical={`https://theshineeducation.com${location.pathname}`}
      />
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <section className="border border-brand-border bg-gradient-to-r from-brand-teal/10 via-white to-brand-teal-light rounded-sm p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <Button to="/counselling" variant="primary" size="md">
                Get Free Counselling
              </Button>
              <Button to="/compare" variant="outline" size="md">
                Compare Institutes
              </Button>
            </div>
          </section>
