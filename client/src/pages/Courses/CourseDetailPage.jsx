...
import { Clock, GraduationCap, CheckCircle2, ArrowRight, HelpCircle, Briefcase, Award } from 'lucide-react';
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    async function loadCourseData() {
      setLoading(true);
      const res = await apiService.getCourseBySlug(slug || 'btech');
      if (res.success) {
        setCourse(res.data);
        // Find colleges offering this stream
        const collegesRes = await apiService.getColleges({ stream: res.data.shortName });
        if (collegesRes.success) {
          setRelatedColleges(collegesRes.data.slice(0, 3));
        }
        // Other courses
        const allCoursesRes = await apiService.getCourses();
        if (allCoursesRes.success) {
          setOtherCourses(allCoursesRes.data.filter((c) => c.slug !== slug).slice(0, 4));
        }
      }
      setLoading(false);
      window.scrollTo(0, 0);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      <SEO
        title={`${course.name}: Eligibility, Syllabus, Top Colleges, Fees & Placements 2026`}
        description={`Comprehensive 2026 course guide for ${course.name}. Understand entrance tests (${course.entranceExams.slice(0, 3).join(', ')}), average CTC ${course.averageSalary}, specializations and top colleges.`}
        canonical={`https://theshineeducation.com/courses/${course.slug}`}
        schema={{
          "@type": "Course",
          "name": course.name,
          ...
          "provider": {
            ...
            "sameAs": "https://theshineeducation.com"
          }
        }}
      />
