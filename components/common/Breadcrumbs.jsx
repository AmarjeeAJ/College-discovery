    ...
    <nav aria-label="Breadcrumb" className="py-3 text-xs sm:text-sm text-brand-gray border-b border-brand-border/60 mb-6">
      <ol className="flex items-center flex-wrap gap-1.5 list-none p-0 m-0">
        <li className="flex items-center">
          <Link href="/" className="inline-flex items-center gap-1 hover:text-brand-teal transition-colors text-brand-gray">
        ...
        {items.map((item, index) => {
          ...
          return (
            <li key={index} className="flex items-center">
              ...
              {href && !isLast ? (
                <Link href={href} className="hover:text-brand-teal transition-colors text-brand-gray">
                  {item.label}
                </Link>
              ) : (
